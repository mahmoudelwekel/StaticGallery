...
reqCategories.open('GET', 'Categories/Categories.json?nocache=' + (new Date()).getTime()); reqCategories.onload = function ()
{
    ...
    {
        document.getElementById("catBtns").insertAdjacentHTML('beforeend', "<a href='index.html?cat=" + i + "' class='btn btn-sm font-weight-bolder btn-outline-primary m-1' role='button' aria-pressed='true'>" + Categories[i].name + "</a>")
    }
};
reqCategories.send();
    ...
    reqinfo.onload = function ()
    {
        ...
        if (cat != '' && cat != null)
        {
            ...
            reqCategories.onload = function () {
                var Categories = JSON.parse(reqCategories.responseText);
                cate_descrip.insertAdjacentHTML('beforeend', "<div class='alert mt-3 pt-4 alert-info shadow text-right' dir='rtl'><h5 class=' font-weight-bold'> تفاصيل عن القسم :</h5>  " + Categories[cat].description + "</div>");
